...
#define PREFIX "/home/dane/Desktop/5grams-pruned/"
#define OUTFILE "/home/dane/Desktop/top_100_5grams_with_awful"
#define METAFILE "/mnt/storage/data_mining_project/data/5grams-info/eng-us-all-totalcounts.txt"
#define REGEXSTR "^awful.*|.* awful.*"
...
void ll_insert(linked_list *l, node *n)
{
    ...
    if(cur == NULL) {
        ...
        l->tail = n;
        l->count = 1;
        l->max = abs_val(n->occurance_rate);
        l->min = abs_val(n->occurance_rate);
        return;
    }

    while(1) {
        if(abs_val(n->occurance_rate) >= abs_val(cur->occurance_rate)) {
            n->prev = cur->prev;
            n->next = cur;
            if(cur->prev != NULL) {
                cur->prev->next = n;
    ...
    if(l->count > 100) {
        node *byebye = l->tail;
        l->tail = byebye->prev;
        l->min = abs_val(l->tail->occurance_rate);
        l->tail->next = NULL;
        l->count -= 1;
        free_node(byebye);
        printf("min: %s %d %e\nmax: %s %d %e\n", l->tail->gram, l->tail->year, l->tail->occurance_rate, l->head->gram, l->head->year, l->head->occurance_rate);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    for(int i = 0; i < paths.count; i++) {
        ...
        while((ptr = gzgets(gz_in, line, LINESIZE)) != NULL) {
            ...
            if(!regexec(&regex, line, 0, NULL, 0)) {
                ...
                if(ll.count < 100 || normed_count > ll.min) {
                    node *n = make_node(new_ngram, normed_count, year);
                    ll_insert(&ll, n);
                }
            }
    ...
    while(n != NULL) {
        fprintf(outf, "%s %d %e\n", n->gram, n->year, n->occurance_rate);
        n = n->next;
    }
